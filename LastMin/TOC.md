...
* **Concatenation**: Joining two strings. Example: w1 = ab, w2 = ba → w1.w2 = abba
* **Length (|w|)**: Number of symbols in a string. Example: |abba| = 4
* **Reverse (w^R)**: Reversing the order of symbols. Example: w = abba → w^R = abba

#### 4. Prefix, Suffix, and Substring

* **Prefix**: Any leading part of a string. Example: For w = abba, prefixes are {ε, a, ab, abb, abba}
* **Suffix**: Any trailing part of a string. Example: For w = abba, suffixes are {ε, a, ba, bba, abba}
* **Substring**: Any continuous part of a string. Example: For w = abba, substrings are {ε, a, b, ab, ba, bb, abb, bba, abba}

#### 5. Language

* **Language (L)**: A set of strings over an alphabet. Example: L = {w ∈ Σ* | w starts with a and ends with b}. For Σ = {a, b}, L = {ab, aab, abb, ...}
...
* Chomsky Hierarchy defines the classification of languages based on generative power of grammars.
...
* Transition function: δ: Q × Σ → Q. It maps a state and input symbol to a single next state.
...
* Divide states into final and non-final sets (P0).
* Iteratively refine partitions (Pk) until no change.
* Merge states in same partition to form minimized DFA.
* Two states are distinguishable if transitions on any input lead to different partitions.

### Moore and Mealy Machine

**Moore Machine:** Output depends on the state only. Output produced when entering a state. Example: Output = {q0 → 0, q1 → 1}. Representation: M=(Q,Σ,Δ,δ,λ,q0), where λ is the output function.

**Mealy Machine:** Output depends on state and input. Output produced during transitions. Example: Transition q0 → q1 on a produces 1. Representation: M=(Q,Σ,Δ,δ,λ,q0), where λ is the transition-based output function.
...
* Infinite languages are regular if they follow patterns. Example: L = {a^n | n ≥ 0} → Regular. L = {a^n b^m | n,m ≥ 0} → Regular.

### Closure Properties

* Regular languages are closed under union, concatenation, intersection (finite sets).
* Not closed under subset or some infinite operations.

### Pumping Lemma Test

* Purpose: To prove that a language is not regular.
* Any string w with |w| ≥ p can be split as xyz with xy^iz ∈ L for all i ≥ 0, |y| > 0, |xy| ≤ p.

### Arden's Theorem

* Solves equations R=Q+RP. Solution: R=QP* if P does not contain ε. Example: R = a+Rb → R = a(b*).
...
* CFG generates context-free languages where each production follows V → (V ∪ T)*.
* LHS: One variable (non-terminal). RHS: Combination of terminals and/or non-terminals.

### Derivations

* Linear Derivation: Left Most Derivation (LMD), Right Most Derivation (RMD).
* Non-Linear Derivation: Parse Tree / Derivation Tree.

### Types of CFG

* Ambiguous Grammar: Multiple parse trees for same string.
* Unambiguous Grammar: Exactly one parse tree per string.
* Left-Recursive: A→Aα. Example: S→Sa∣b.
* Right-Recursive: A→αA. Example: S→aS∣b.
...
* Extends FA using stack for additional memory.
* Recognizes Context-Free Languages (CFLs).

### Types of PDA

1. Deterministic PDA (DPDA): Recognizes DCFL. Cannot handle ambiguity.
2. Non-Deterministic PDA (NPDA): Recognizes all CFLs. Can handle ambiguity.
...
* TM is a mathematical model used to define computation.
...
* TM simulates FA and PDA; more powerful than DFA, NFA, PDA.

### Recursive and Recursively Enumerable Languages

* Recursive (Decidable): TM halts on every input and decides membership.
* Recursively Enumerable (RE, Semi-Decidable): TM halts for inputs in the language; may loop for others.
...
* Countable: All Turing Machines.
* Uncountable: All possible languages; more languages than TMs.
